<?php
// ============================================================
//  book_appointment.php  — Employee Portal
//  Saves a new appointment to the `appointments` table.
//  patient_type is fixed as 'employee'.
// ============================================================
session_start();
require_once "../config/db.php";   // provides $c (mysqli connection)

// ── GUARD: must be logged in as employee ─────────────────────
...
$role       = 'employee';
$profile_id = $_SESSION['profile_id'];   // employee_id
$user_id    = $_SESSION['user_id'];

// ── PRE-FILL: load employee profile to auto-fill the form ─────
$q = mysqli_prepare($c,
    "SELECT CONCAT(last_name, ', ', first_name, ' ', IFNULL(middle_name,'')) AS full_name,
            employee_number, email, contact_number,
            position_designation AS program,
            department AS year_level
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_appointment'])) {
    ...
    $id_number      = trim($_POST['id_number']      ?? $prefill['employee_number'] ?? '');
    ...
    if (empty($id_number))
        $field_errors['id_number'] = "Employee number is required.";
    ...
    <style>
        ...
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .confirm-modal-header {
            ...
            padding: 28px 32px 22px; color: white; text-align: center;
        }
        ...
        .confirm-item span { font-size: 14px; font-weight: 600; color: var(--text-dark); }
        .confirm-reason-box {
        ...
        .confirm-reason-box p { font-size: 13px; color: var(--text-dark); line-height: 1.55; margin: 0; }
        .confirm-notice {
        ...
        .success-modal-body .modal-title { font-size: 22px; font-weight: 700; color: var(--text-dark); margin-bottom: 8px; }
        .success-modal-body .modal-subtitle { font-size: 14px; color: var(--text-gray); line-height: 1.6; margin-bottom: 20px; }
        .modal-ref {
            display: inline-block;
            background: var(--primary-soft); border: 1px solid #fca5a5;
            color: var(--primary); font-size: 15px; font-weight: 700;
            padding: 9px 28px; border-radius: 50px; margin-bottom: 24px; letter-spacing: .5px;
        }
        .modal-details {
        ...
        .btn-modal-primary {
            ...
            cursor: pointer; transition: background .25s; font-family: 'Poppins',sans-serif;
        }
...
<main class="main-content">
    ...
    <!-- PHP → JS bridge for success modal -->
    <div id="appt-result"
         data-save="<?= $save_ok ? '1' : '0' ?>"
         data-ref="<?= $save_ok ? 'APT-' . str_pad($new_id, 5, '0', STR_PAD_LEFT) : '' ?>"
         data-category="<?= htmlspecialchars($service_category) ?>"
         data-date="<?= htmlspecialchars($appointment_date) ?>"
         data-time="<?= htmlspecialchars($appointment_time) ?>"
        ...
        <div class="form-card-body">
            ...
            <form method="POST" action="book_appointment.php" id="appointmentForm" novalidate>
                ...
                <div class="form-grid-3">
                    ...
                    <!-- Employee Number -->
                    <div class="form-group">
                        <label for="id_number">Employee Number <span class="req">*</span></label>
                        <input type="text" name="id_number" id="id_number"
                               value="<?= fval('id_number', $P, $prefill, 'employee_number') ?>"
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email_input" id="email_input"
                               ...
                               placeholder="employee@pup.edu.ph"
                ...
                <div class="form-grid-3" style="margin-top:20px;">
                    ...
                    <div class="form-group">
                        <label for="program_input">Position / Designation</label>
    ...
    switch (id) {
        ...
        case 'appointment_time':
            ...
            if (val < '08:00' || val > '17:00') {
                showError(id, 'Please choose a time between 8:00 AM and 5:00 PM.'); return false;
            }
            break;
        ...
        case 'full_name':
            ...
            break;
        case 'id_number':
            if (!val) { showError(id, 'Employee number is required.'); return false; }
            break;
        case 'email_input':
...
function validateAllFields() {
    ...
    if (!terms.checked) {
        ...
        if (!document.getElementById('terms-error-live')) {
            const div = document.createElement('div');
...
function getBotResponse(message) {
    ...
    if (m.includes('requirement') || m.includes('document') || m.includes('need'))
        return "You'll need your <strong>employee number, a valid email, contact number</strong>, and a clear reason for your visit. All marked fields are required.";
