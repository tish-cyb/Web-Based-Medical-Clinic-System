<?php
// ============================================================
//  medical_history.php  — Employee Portal
//  Pulls ALL medical history from the DB for the logged-in
//  employee using the vw_employee_medical_history view, then
...
// ── 1. EMPLOYEE PROFILE (for name / ID display) ──────────────
$q = mysqli_prepare($c,
    "SELECT CONCAT(last_name, ', ', first_name, ' ', IFNULL(middle_name,'')) AS full_name,
            employee_number, department, position_designation, employment_type
...
$q2 = mysqli_prepare($c,
    "SELECT record_date, record_type, sub_type, diagnosis_purpose,
     ...
     FROM   vw_employee_medical_history
     WHERE  employee_id = ?
     ORDER  BY record_date DESC, source_id DESC");
mysqli_stmt_bind_param($q2, "i", $employee_id);
...
$q3 = mysqli_prepare($c,
    "SELECT cr.record_id, cr.consultation_date, cr.consultation_time,
     ...
     WHERE  cr.employee_id = ? AND cr.patient_type = 'employee'");
mysqli_stmt_bind_param($q3, "i", $employee_id);
...
$q4 = mysqli_prepare($c,
    "SELECT he.exam_id, he.exam_date, he.school_year_semester,
     ...
     WHERE  he.employee_id = ? AND he.patient_type = 'employee'");
mysqli_stmt_bind_param($q4, "i", $employee_id);
...
$q5 = mysqli_prepare($c,
    "SELECT lr.lab_id, lr.request_date, lr.status,
     ...
     WHERE  lr.employee_id = ? AND lr.patient_type = 'employee'");
mysqli_stmt_bind_param($q5, "i", $employee_id);
...
$q6 = mysqli_prepare($c,
    "SELECT mc.clearance_id, mc.issue_date, mc.valid_until,
     ...
     WHERE  mc.employee_id = ? AND mc.patient_type = 'employee'");
mysqli_stmt_bind_param($q6, "i", $employee_id);
...
$q7 = mysqli_prepare($c,
    "SELECT cert.certificate_id, cert.certificate_type,
     ...
     WHERE  cert.employee_id = ? AND cert.patient_type = 'employee'");
mysqli_stmt_bind_param($q7, "i", $employee_id);
...
$full_name        = trim($profile['full_name']           ?? 'Employee');
$employee_number  = $profile['employee_number']          ?? '—';
$department       = $profile['department']               ?? '—';
$position         = $profile['position_designation']     ?? '—';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Employee Portal — Medical History</title>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .lab-tag  { background:#dbeafe; color:#1e40af; padding:5px 14px; border-radius:20px; font-size:12px; font-weight:600; }
...
<main class="main-content">
    <div class="page-header">
        ...
        <p>All medical records for <strong><?= esc($full_name) ?></strong> &nbsp;&middot;&nbsp; <?= esc($employee_number) ?> &nbsp;&middot;&nbsp; <?= esc($department) ?></p>
    </div>
...
<div class="modal-overlay" id="detailsModal">
    <div class="modal-box">
        ...
        <!-- Scrollable body -->
        <div class="modal-scroll" id="modal-body-content"></div>
...
function buildBody(r) {
    switch (r.type) {
        ...
        case 'Laboratory Request':
            ...
            <div class="d-grid-2">${field('Request Status', r.status)}</div>
...
function getBotResponse(message) {
    ...
    if (m.includes('record') || m.includes('history'))
        return `You have <strong>${totalRec} medical record${totalRec !== 1 ? 's' : ''}</strong> on file. Click <em>View Details</em> on any row to see the full details.`;
    ...
    if (m.includes('download') || m.includes('print'))
        return "To print a record, open it using <em>View Details</em> and use your browser's print function (Ctrl+P / Cmd+P).";
    if (m.includes('diagnosis') || m.includes('treatment'))
        return "Click <em>View Details</em> next to any consultation record to see the full diagnosis, treatment, and medications prescribed.";
