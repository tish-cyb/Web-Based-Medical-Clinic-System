...
$q2 = mysqli_prepare($c,
    "SELECT v.cert_id, v.certificate_type, v.issue_date,
     ...
     FROM   vw_student_certificates v
     LEFT JOIN (
         SELECT mc.staff_id, mc.clearance_id AS sid, 'medical_clearances' AS tbl
         FROM medical_clearances mc WHERE mc.student_id = ?
         UNION ALL
         SELECT cert.staff_id, cert.certificate_id, 'medical_certificates'
         FROM medical_certificates cert WHERE cert.student_id = ?
...
mysqli_stmt_bind_param($q2, "iii", $student_id, $student_id, $student_id);
...
<head>
    ...
    <title>Student Portal — Medical Certificates</title>
