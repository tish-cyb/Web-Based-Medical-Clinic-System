...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_update'])) {
    ...
    $upd = mysqli_prepare($c,
        "UPDATE students SET email=?, contact_number=?, address=?, civil_status=?,
         emergency_contact_name=?, emergency_contact_number=?, updated_at=NOW()
         WHERE student_id=?");
    mysqli_stmt_bind_param($upd, "ssssssi",
        $email, $contact_number, $address, $civil_status, $ec_name, $ec_number, $student_id);
...
$q = mysqli_prepare($c,
    "SELECT s.student_number, s.last_name, s.first_name, s.middle_name,
            s.sex, s.date_of_birth, s.civil_status, s.contact_number,
            s.email, s.address, s.campus, s.college, s.program,
            s.year_level, s.school_year, s.emergency_contact_name,
            s.emergency_contact_number, u.email AS login_email
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        /* ── Sidebar ── */
        .sidebar { width:275px; background:linear-gradient(180deg,#860303 3%,#B21414 79%,#940000 97%); color:white; position:fixed; height:100vh; left:0; top:0; display:flex; flex-direction:column; box-shadow:4px 0 10px rgba(0,0,0,.1); z-index:1000; }
        ...
        .nav-item { padding:16px 32px; transition:all .3s; font-size:15px; font-weight:500; border-left:4px solid transparent; display:flex; align-items:center; gap:12px; color:rgba(255,255,255,.9); text-decoration:none; }
        ...
        .chatbot-toggle { width:60px; height:60px; background:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:28px; box-shadow:0 4px 8px rgba(0,0,0,.2); cursor:pointer; transition:transform .3s; }
        ...
        /* Active editable */
        .form-group input:not([readonly]):not(.locked):focus, .form-group select:not(:disabled):focus, .form-group textarea:not([readonly]):focus { border-color:var(--primary); box-shadow:0 0 0 3px rgba(127,29,29,.07); background:white; }
        ...
        .emergency-banner { display:flex; align-items:center; gap:10px; padding:10px 14px; background:var(--primary-soft); border:1px solid #fca5a5; border-radius:8px; margin-bottom:16px; }
        ...
        <div class="profile-card">
            ...
            <div class="section-body">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label>Student Number <i class="bi bi-lock-fill lock-icon"></i></label>
                        <input type="text" value="<?= val($p['student_number'] ?? '') ?>" class="locked" readonly tabindex="-1">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <label>College <i class="bi bi-lock-fill lock-icon"></i></label>
                        <input type="text" value="<?= val($p['college'] ?? '') ?>" class="locked" readonly tabindex="-1">
                    </div>
                    <div class="form-group">
                        <label>Program / Course <i class="bi bi-lock-fill lock-icon"></i></label>
                        <input type="text" value="<?= val($p['program'] ?? '') ?>" class="locked" readonly tabindex="-1">
                    </div>
                    <div class="form-group">
                        <label>Year Level <i class="bi bi-lock-fill lock-icon"></i></label>
                        <input type="text" value="<?= val($p['year_level'] ?? '') ?>" class="locked" readonly tabindex="-1">
                    </div>
                    <div class="form-group">
                        <label>School Year <i class="bi bi-lock-fill lock-icon"></i></label>
                        <input type="text" value="<?= val($p['school_year'] ?? '') ?>" class="locked" readonly tabindex="-1">
                    </div>
                </div>
            </div>
